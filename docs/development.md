...
`.env.secrets` is yours alone — it's gitignored, and you create it from the committed example:
...
Seeding indexes content through those embeddings, so both keys must be set before `make db_seed`.
