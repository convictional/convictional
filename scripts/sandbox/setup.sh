...
if [ "$created" = 1 ]; then
  ...
  $SANDBOX_SSH bash -c 'cat > ~/app/.env.secrets' < .env.sandbox

  step "Installing dependencies"
  $SANDBOX_SSH bash -lc 'cd ~/app && make install ARGS="--chromium"'

  step "Creating databases"
  $SANDBOX_SSH bash -lc 'cd ~/app && make db_create'
  $SANDBOX_SSH bash -lc 'cd ~/app && ENV=test make db_create'
