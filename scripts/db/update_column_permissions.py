...
UPDATE_COLUMN_PERMISSIONS_SQL = """
    ...
    BEGIN
        ...
        LOOP
            ...
            FOREACH restricted_user IN ARRAY ARRAY[{restricted_users}] LOOP
...
async def update_column_permissions():
    if not settings.restricted_db_users:
        print(red_text("No restricted users configured. Set RESTRICTED_DB_USERS to a comma-separated list of roles."))
        return 1

    restricted_users = ", ".join("'" + user.replace("'", "''") + "'" for user in settings.restricted_db_users)
    try:
        ...
        await connection.execute(UPDATE_COLUMN_PERMISSIONS_SQL.format(restricted_users=restricted_users))
