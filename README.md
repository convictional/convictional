...
Copy `.env.secrets.example` to `.env.secrets` and fill in keys for the two model providers:

```bash
cp .env.secrets.example .env.secrets
```

`ANTHROPIC_API_KEY` drives inference and `OPENAI_API_KEY` drives the embeddings behind search — seeding
indexes content, so both are needed before `make db_seed`. See
[development](docs/development.md#configuration) for where to get them.
